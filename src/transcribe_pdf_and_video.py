...
def convert_video_to_audio(video_path: str) -> str:
    """Converte vídeo para áudio usando ffmpeg com otimizações para arquivos grandes"""
    ...
    cmd = [
        ...
        '-ac', '1',  # Mono
        '-af', 'volume=0.8',  # Reduzir volume para evitar clipping
        '-threads', '2',  # Usar apenas 2 threads para economizar CPU
        '-y',  # Sobrescrever
        audio_path
    ]
    
    try:
        logger.info(f"Iniciando conversão de vídeo para áudio: {video_path}")
        result = subprocess.run(cmd, capture_output=True, text=True, check=True, timeout=600)  # Timeout de 10 minutos
        logger.info(f"Conversão concluída: {audio_path}")
        return audio_path
        ...
        raise HTTPException(status_code=500, detail="Timeout na conversão do vídeo")

def transcribe_video_file(video_path: str, language: str = "pt") -> dict:
    """Transcreve arquivo de vídeo usando Whisper com otimizações para arquivos grandes"""
    ...
    try:
        ...
        transcribe_options = {
            ...
            "compression_ratio_threshold": 2.4,  # Otimização de qualidade
            "no_speech_threshold": 0.6,  # Detectar melhor silêncio
        }
        
        if language and language != "auto":
            transcribe_options["language"] = language
        
        logger.info(f"Iniciando transcrição com Whisper Tiny...")
...
async def transcribe_video(
    ...
    # Limite de tamanho (200MB) - otimizado para VPS 8GB
    if len(content) > 200 * 1024 * 1024:
        raise HTTPException(
            status_code=413,
            detail="Arquivo muito grande. Máximo: 200MB"
        )
...
@app.post("/transcribe-large-video")
async def transcribe_large_video(
    file: UploadFile = File(...),
    language: Optional[str] = "pt"
):
    """
    Endpoint otimizado para vídeos grandes (até 500MB)
    
    - **file**: Arquivo de vídeo grande (mp4, avi, mov, mkv, etc.)
    - **language**: Código do idioma (pt, en, es, etc.)
    """
    ...
    # Limite aumentado para vídeos grandes (500MB)
    if len(content) > 500 * 1024 * 1024:
        raise HTTPException(
            status_code=413,
            detail="Arquivo muito grande. Máximo: 500MB para vídeos grandes"
        )
    ...
    try:
        ...
        logger.info(f"Processando vídeo grande: {file.filename} ({len(content) / (1024*1024):.1f} MB)")
        
        # Transcrever vídeo com otimizações especiais
        result = transcribe_video_file(temp_file_path, language)
        ...
        logger.info("Transcrição de vídeo grande concluída com sucesso")
        return response
        
    except Exception as e:
        logger.error(f"Erro na transcrição de vídeo grande: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erro na transcrição: {str(e)}")
